# Symfony 6/7 – Prod Render (PostgreSQL) – sans start.sh
FROM php:8.2-cli
RUN apt-get update && apt-get install -y --no-install-recommends git unzip postgresql-client libicu-dev libpq-dev
RUN docker-php-ext-configure intl
RUN docker-php-ext-install -j"$(nproc)" intl pdo pdo_pgsql
RUN rm -rf /var/lib/apt/lists/*
RUN php -r "if (!extension_loaded('intl')) { throw new Exception('intl not loaded'); } echo '✅ intl extension loaded successfully';"
