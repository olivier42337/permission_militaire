FROM php:8.2-cli
RUN apt-get update && apt-get install -y --no-install-recommends git unzip sqlite3 libsqlite3-dev libicu-dev icu-devtools
RUN docker-php-ext-configure intl --with-icu-dir=/usr
RUN docker-php-ext-install -j"$(nproc)" intl
RUN docker-php-ext-install -j"$(nproc)" pdo_sqlite
RUN rm -rf /var/lib/apt/lists/*
RUN php -r "if (!extension_loaded('intl')) { throw new Exception('intl not loaded'); } echo '✅ intl OK';"
...
COPY . .
RUN echo "DATABASE_URL=sqlite:///%kernel.project_dir%/var/data.db" >> .env && composer dump-env prod
